...
The `@web3auth/ethereum-mpc-provider` is a Web3Auth provider that simplifies interaction with the
EIP1193 based EVM blockchains. It is used to interact with the EVM blockchain and perform various
operations like getting user's account, balance, signing a transaction, sending a transaction etc.
